...
int custom_total_items=0;
    String url = "jdbc:postgresql://localhost:5432/dspace";
		...
		String sqlstr = "select count(*) from collection2item ";
		...
		try
		{
			...
			while(rs.next())
			{
				custom_total_items = rs.getInt(1);
...
else if( qResults != null)
{
	...
	int custom_lastpage=(int)custom_total_items/10+1;
    pageTotal=custom_lastpage;
	pageLast=custom_lastpage;
	
	if(request.getParameter("filterquery")!=null||request.getParameter("filter_value_1")!=null||request.getParameter("submit_filter_remove_1")!=null){
	
   pageTotal   = ((Long)request.getAttribute("pagetotal"  )).longValue();
   custom_lastpage=(int)pageTotal;
pageLast    = ((Long)request.getAttribute("pagelast"   )).longValue();
custom_total_items=(int)qResults.getTotalSearchResults();
}
	
	
	//out.print(pageTotal+"aa");
...
%>
	...
	<div class="alert alert-info"><fmt:message key="jsp.search.results.results">
        ...
        <fmt:param><%=custom_total_items%></fmt:param>
	...
	for( long q = pageFirst; q <= custom_lastpage; q++ )
	{
