...
<script>
    $(document).ready(function () {
        ...
        $('#control1').click(function () {
            $('#q1').attr('name', 'addfield_value');
            $('#h1').attr('name', 'addfield_dctype');
        });
        $('#control2').click(function () {
            $('#q2').attr('name', 'addfield_value');
            $('#h2').attr('name', 'addfield_dctype');
        });
...
<%!  
String contextPath;
    String doControlledVocabulary(String ch,String label,String fieldName, String vocabulary) {
        String link = "";
       
       
            link
                    = "<a id='"+ch+"' href='javascript:void(null);' onclick='javascript:popUp(\""
                    + "/jspui/controlledvocabulary/controlledvocabulary1.jsp?ID="
                    + "addfield_value&amp;vocabulary=" + vocabulary + "\")'>"
...
Vector <String> vec_items=new Vector<String>();
int kk=0,kj=0;
String finals="";
                    ...
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br /> <% 
							session.setAttribute("collectionID",collections[i].getID());} %>
                            </td>
                        </tr>
                        <tr>
        ...
        <div class="table-responsive">
            <table class="table" summary="Edit item withdrawn table">
                ...
                <%
                   ...
                   item.addMetadata("compass", "learningOpportunity", "competence", null, "11111111111111AAAAAAAsss");
                item.update();   
                ...
                %>
                <tr>
                    <td headers="t0" class="<%= row %>RowOddCol"><%=dcv[i].schema %>.<%= dcv[i].element %>.<%= (dcv[i].qualifier == null ? "" : dcv[i].qualifier) %></td>
                ...
                finals=dcv[i].schema+"."+dcv[i].element+"."+(dcv[i].qualifier == null ? "" : dcv[i].qualifier);
                if((dcv[i].qualifier!=null)&&(dcv[i].qualifier.equals("competence"))&&kk<1&&(dcv[i].element.equals("learningOpportunitySpecification"))){
                        kk=kk+1;
			
                        ab.append("<tr>")
                        .append("<td style='border-top:0px solid #ddd' headers='t1' class='"+row+"'RowEvenCol'>")
                        .append("Add New Value:<input type='hidden' id='h1'  class='form-control' name='addfield_dctype1' value=157 />")
                        .append("</td><td style='border-top:0px solid #ddd' headers='t3' class='"+row+"RowOddCol'>")
.append(" <textarea id='q1' class='form-control' name='addfield_value1' rows='3' cols='50'></textarea>")
.append(doControlledVocabulary("control1","labelkl","addfield_value1", "test"))
.append(" </td>")


			
.append("<td style='border-top:0px solid #ddd' headers='t5' class='"+row+"RowOddCol'>")
.append("<button class='btn btn-default' name='submit_addfield' value='<fmt:message key='jsp.tools.general.add'/>'")
...
out.write(ab.toString());
                        ...
                        if((dcv[i].qualifier!=null)&&(dcv[i].qualifier.equals("level"))&&kj<1&&(dcv[i].element.equals("learningOpportunitySpecification"))){
                        kj=kj+1;
			
                        bb.append("<tr>")
                        .append("<td style='border-top:0px solid #ddd' headers='t1' class='"+row+"'RowEvenCol'>")
                        .append("Add New Value:<input type='hidden' id='h2'  class='form-control' name='addfield_dctype2' value=148 />")
                        .append("</td><td style='border-top:0px solid #ddd' headers='t3' class='"+row+"RowOddCol'>")
.append(" <textarea id ='q2' class='form-control' name='addfield_value2' rows='3' cols='50'></textarea>")
.append(doControlledVocabulary("control2","labelkl","addfield_value2", "test"))
.append("<p>ss</p>")
.append(" </td>")
